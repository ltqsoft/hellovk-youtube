...
void Application::createCmds()
{
    ...
    for(uint32_t i=0; i<minImageCount; i++)
    {
        ...
        vkCmdBindVertexBuffers(cmd, 0, 1, &vertexBuffer.id, &offset);
        vkCmdBindIndexBuffer(cmd, indexBuffer.id, 0, VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(cmd, indexCount, SO_LUONG_INSTANCE, 0,0,0);
