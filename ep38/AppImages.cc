...
Image Application::loadTexture(const char* const filePath)
{
    Image ret = {};
    ...
    vkfunc(result, "Loi: tao sampler khong thanh cong!",
        vkCreateSampler(device, &samplerInfo, nullptr, &ret.sampler)
    );
    ...
    vkCmdCopyBufferToImage(copyCmd, temp.id, ret.id, barrier.newLayout,
        1, &region
    );
...
{
    texture = this->loadTexture(TEXTURE_FILE_PATH);
}
