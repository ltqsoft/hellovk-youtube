...
void Application::createCmdPools()
{
    ...
    std::cout << "Tao cmd pool thanh cong.\n";
}
...
void Application::createCmds()
{
    ...
    for(uint32_t i=0; i<minImageCount; i++)
    {
        ...
        VkClearValue clearValues[SO_LUONG_ATTACHMENT];
        clearValues[ATTACH_COLOR] = mau_den;
        clearValues[ATTACH_DEPTH].depthStencil.depth = 1.0f;
        ...
        rpBegInfo.clearValueCount = SO_LUONG_ATTACHMENT;
        rpBegInfo.pClearValues = clearValues;
        ...
        for(Mesh& m : meshes) {
            ...
            vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                pipelineLayout, 0, 1, &m.descriptorSet, 0, nullptr
            );
        
            VkDeviceSize offset = 0;
            vkCmdBindVertexBuffers(cmd, 0, 1, &m.vertexBuffer.id, &offset);
            vkCmdBindIndexBuffer(cmd, m.indexBuffer.id, 0, VK_INDEX_TYPE_UINT32);
            vkCmdDrawIndexed(cmd, m.indexCount, SO_LUONG_INSTANCE, 0,0,0);
        }
