...
void Application::createGraphicsPipeline()
{
    ...
    vertexAttribs.resize(SO_LUONG_VERTEX_ATTR, {});

    const VkFormat format_vec2 = VK_FORMAT_R32G32_SFLOAT;
    const VkFormat format_vec3 = VK_FORMAT_R32G32B32_SFLOAT;
    
    vertexAttribs[ATTR_POSITION].format = format_vec3;
    vertexAttribs[ATTR_POSITION].location = ATTR_POSITION;
    vertexAttribs[ATTR_POSITION].offset = offsetof(Vertex, position);

    vertexAttribs[ATTR_COLOR].format = format_vec3;
    vertexAttribs[ATTR_COLOR].location = ATTR_COLOR;
    vertexAttribs[ATTR_COLOR].offset = offsetof(Vertex, color);

    vertexAttribs[ATTR_TEXTURE_COORD].format = format_vec2;
    vertexAttribs[ATTR_TEXTURE_COORD].location = ATTR_TEXTURE_COORD;
    vertexAttribs[ATTR_TEXTURE_COORD].offset = offsetof(Vertex, textureCoord);

    vertexAttribs[ATTR_NORMAL].format = format_vec3;
    vertexAttribs[ATTR_NORMAL].location = ATTR_NORMAL;
    vertexAttribs[ATTR_NORMAL].offset = offsetof(Vertex, normal);
